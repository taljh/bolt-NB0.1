...
const formSchema = z.object({
  ...
  fabric_secondary_cost: z.number().nullable(),
  has_turha: z.boolean().default(false),
  turha_main_cost: z.number().nullable(),
  has_secondary_turha: z.boolean().default(false),
  turha_secondary_cost: z.number().nullable(),
...
// دالة جلب إعدادات المشروع
const fetchProjectSettings = async (supabase: any) => {
...
export default function ProductPricingForm({ productId }: ProductPricingFormProps) {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        // جلب إعدادات المشروع
        const projectSettings = await fetchProjectSettings(supabase);
        
        if (projectSettings) {
          // تعيين القيم الافتراضية من إعدادات المشروع
          setValue('profit_margin', projectSettings.default_profit_margin);
          setValue('target_audience', projectSettings.target_audience);
        }
        ...
        // إذا لم يوجد سجل للتسعير، نقوم بإنشائه مع القيم الافتراضية من إعدادات المشروع
        if (pricingResponse.error || !pricingResponse.data) {
          console.log("📭 No pricing record found, creating one...");
        
          const { error: insertError, data: newPricing } = await supabase
            .from("pricing_details")
            .upsert({
              product_id: productId,
              user_id: user.id,
              target_audience: projectSettings?.target_audience || "medium", // القيمة الافتراضية من إعدادات المشروع
              profit_margin: projectSettings?.default_profit_margin || 30
            }, { onConflict: 'product_id' })
            .select()
            .single();
        
          if (insertError || !newPricing) {
            console.error("❌ Failed to create pricing record:", insertError);
            toast({
              title: "خطأ في إنشاء سجل التسعير",
              description: "تعذر إنشاء سجل للتسعير لهذا المنتج",
              variant: "destructive",
            });
          } else {
            // تعبئة القيم حتى لو كانت null
            setValue('fabric_main_cost', newPricing.fabric_main_cost);
            setValue('fabric_secondary_cost', newPricing.fabric_secondary_cost);
            setValue('turha_main_cost', newPricing.turha_main_cost);
            setValue('turha_secondary_cost', newPricing.turha_secondary_cost);
            setValue('tailoring_cost', newPricing.tailoring_cost);
            setValue('packaging_cost', newPricing.packaging_cost);
            setValue('delivery_cost', newPricing.delivery_cost);
            setValue('extra_expenses', newPricing.extra_expenses);
            setValue('profit_margin', newPricing.profit_margin);
            setValue('target_audience', newPricing.target_audience);
            setValue('has_secondary_fabric', Boolean(newPricing.fabric_secondary_cost));
            setValue('has_turha', Boolean(newPricing.turha_main_cost));
            setValue('has_secondary_turha', Boolean(newPricing.turha_secondary_cost));
            
            // انتظار بسيط لضمان كتابة البيانات
            await new Promise(resolve => setTimeout(resolve, 300));
          }
        }

        // التحقق من ملكية المنتج
        if (productResponse.data.user_id !== user.id) {
        ...
        // تعبئة بيانات التسعير إذا وُجد سجل، حتى لو كان مضافًا حديثاً أو جزئيًا
    ...
    fetchProduct();
  }, [productId, setValue, supabase, router, toast]);
  ...
  useEffect(() => {
    ...
    const optionalFields = {
      fabric_secondary_cost: !watchRequiredFields.has_secondary_fabric || Boolean(watchRequiredFields.fabric_secondary_cost),
      turha_main_cost: !watchRequiredFields.has_turha || Boolean(watchRequiredFields.turha_main_cost),
      turha_secondary_cost: !watchRequiredFields.has_secondary_turha || Boolean(watchRequiredFields.turha_secondary_cost)
    };
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (userError) {
        ...
        toast({
          title: "❌ خطأ في التحقق من المستخدم",
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <Label>
                    ...
                    <Badge className="ml-2" variant="secondary">مطلوب</Badge>
                  </Label>
            ...
            <div className="bg-gradient-to-r from-indigo-500/10 via-violet-500/10 to-indigo-500/10 px-6 py-4">
              <h3 className="text-lg font-bold flex items-center gap-2 text-gray-800">
                <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-indigo-100 text-indigo-700">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-plus-circle"><circle cx="12" cy="12" r="10"/><path d="M8 12h8"/><path d="M12 8v8"/></svg>
                </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div className="group">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <Badge className="bg-gradient-to-r from-amber-100 to-amber-200 text-amber-700 border-amber-200 mr-2" variant="secondary">مطلوب</Badge>
                  </div>
            ...
            <div className="bg-gradient-to-r from-purple-500/10 via-indigo-500/10 to-purple-500/10 px-6 py-4">
              <h3 className="text-lg font-bold flex items-center gap-2 text-gray-800">
                <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-purple-100 text-purple-700">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-arrow-up-right"><path d="M7 7h10v10"/><path d="M7 17 17 7"/></svg>
                </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="group">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <Badge className="bg-gradient-to-r from-green-100 to-green-200 text-green-700 border-green-200 mr-2" variant="secondary">من إعدادات المشروع</Badge>
                  </div>
                  <div className="relative transition-all duration-300 group-hover:scale-[1.02]">
                    <Select
      ...
      <div className="lg:sticky lg:top-6 lg:h-fit space-y-4">
        ...
        <CostDetailsCard
          ...
          fixedCosts={0} // سيتم تحديثه لاحقاً من إعدادات المشروع
